...
Curation (`/curation`, spec `design/CURATION_SPEC.md`): committee members, per `useCommittee` (`GET /committee`, fails closed), get the Curation tab and review collections in the item editor with `?reviewing=true`, which is read-only for everyone. Committee chain writes go through `lib/collectionApproval` (`Committee.manageCollection`); the approval flow lives in `lib/approveCollection` and Catalyst entities in `lib/catalystEntity`, whose metadata key order is part of the content hash.
