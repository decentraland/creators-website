...
function createScene(nodes: unknown[]): Scene {
  return {
...
function asGLTF(obj: { scene: Scene; animations: unknown[] }): GLTF {
...
describe('validateMaterials', () => {
  ...
  describe('when the material count exceeds the default limit', () => {
    ...
    beforeEach(() => {
      const nodes = Array.from({ length: MAX_MATERIALS_DEFAULT + 1 }, (_, i) =>
        createMeshNode(Three, { materialName: `Mat${i}` })
      )
  ...
  describe('when the category is skin', () => {
    ...
    beforeEach(() => {
      const nodes = Array.from({ length: MAX_MATERIALS_SKIN }, (_, i) =>
        createMeshNode(Three, { materialName: `Mat${i}` })
      )
...
describe('validateTextures', () => {
  ...
  describe('when the material is AvatarSkin_MAT', () => {
    ...
    beforeEach(() => {
      issues = validateTextures(
        asThree(Three),
        createScene([createMeshWithTexture(Three, AVATAR_SKIN_MAT, 'tex1', 2048)])
      )
    })
...
function createAnimations(trackNames: string[]): AnimationClip[] {
  return [
    {
...
describe('validateNoDisallowedObjects', () => {
  ...
  describe('when cameras are present', () => {
    ...
    beforeEach(() => {
      issues = validateNoDisallowedObjects(
        asThree(Three),
        asGLTF({ scene: createScene([new Three.Camera()]), animations: [] })
      )
    })
  ...
  describe('when lights are present', () => {
    ...
    beforeEach(() => {
      issues = validateNoDisallowedObjects(
        asThree(Three),
        asGLTF({ scene: createScene([new Three.Light()]), animations: [] })
      )
    })
  ...
  describe('when animations are present in a wearable', () => {
    ...
    beforeEach(() => {
      issues = validateNoDisallowedObjects(
        asThree(Three),
        asGLTF({ scene: createScene([]), animations: [{ name: 'idle' }] })
      )
    })
  ...
  describe('when no forbidden patterns are found', () => {
    ...
    beforeEach(() => {
      issues = validateMaterialNaming(
        asThree(Three),
        createScene([createMeshNode(Three, { materialName: 'Body_MAT' })])
      )
    })
...
describe('validateArmatureBoneNames', () => {
  ...
  describe('and the skeleton also includes spring bones as extras', () => {
    ...
    beforeEach(() => {
      const mesh = createSkinnedMeshWithJoints(Three, [
        ...AVATAR_BONE_NAMES,
        'Hair.springbone.001',
        'Cape_SpringBone_tail'
      ])
      issues = validateArmatureBoneNames(asThree(Three), createScene([mesh]))
    })
...
describe('getEffectiveTriangleLimit', () => {
  ...
  describe('when hides includes a category without a triangle budget', () => {
    ...
    beforeEach(() => {
      limit = getEffectiveTriangleLimit(WearableCategory.UPPER_BODY, ['head'])
    })
...
describe('validateTriangleCounts with Tris Combiner', () => {
  ...
  describe('when a helmet has 4000 tris and hides hair and hat', () => {
    ...
    beforeEach(() => {
      const scene = createScene([createMeshNode(Three, { triangles: 4000 })])
      issues = validateTriangleCounts(asThree(Three), scene, WearableCategory.HELMET, [
