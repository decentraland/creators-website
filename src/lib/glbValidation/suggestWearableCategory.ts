 ...
 */
function accumulateRegionWeights(Three: ThreeModules, scene: Object3D): Record<AvatarRegion, number> {
  ...
  scene.traverse((node: Object3D) => {
    ...
    const getComponent = (attribute: typeof skinIndex, vertex: number, component: number): number =>
      component === 0
    ...
    for (let v = 0; v < vertexCount; v++) {
      for (let c = 0; c < componentsPerVertex; c++) {
        const boneIdx = getComponent(skinIndex, v, c)
        const weight = getComponent(skinWeight, v, c)
        if (weight <= 0) {
          continue
        }
 ...
 */
export function suggestWearableCategory(Three: ThreeModules, scene: Object3D): WearableCategory | null {
