...
describe('validateEmoteAnimationClipCount', () => {
  ...
  describe('when hasProps is true', () => {
    ...
    describe('and the clip count exceeds the props limit', () => {
      ...
      beforeEach(() => {
        const anims = Array.from({ length: EMOTE_MAX_ANIMATION_CLIPS_WITH_PROPS + 1 }, () =>
          makeAnimation({ duration: 1 })
        )
        issues = validateEmoteAnimationClipCount(anims, true)
      })
...
describe('validateEmoteDeformBoneKeyframes', () => {
  ...
  describe('when prop bones have keyframes in a separate animation clip', () => {
    ...
    beforeEach(() => {
      ...
      issues = validateEmoteDeformBoneKeyframes(asThree(Three), createTraversableScene([avatarBone, propBone]), [
        avatarAnim,
        propAnim
      ])
    })
