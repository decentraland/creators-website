  ...
  if (animation.duration > 0) {
    ...
    if (Math.abs(fps - EMOTE_EXPECTED_FPS) > EMOTE_FPS_TOLERANCE) {
      issues.push({
        ...
        messageKey: 'item_validation.emote_frame_rate',
...
export function validateEmoteMaxFrames(animations: AnimationClip[]): ValidationIssue[] {
  ...
  if (maxFrames > EMOTE_MAX_FRAMES) {
    issues.push({
      ...
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.emote_max_frames',
...
export function validateEmoteAnimationClipCount(animations: AnimationClip[], hasProps: boolean): ValidationIssue[] {
  ...
  if (animations.length > limit) {
    issues.push({
      ...
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.emote_max_clips',
...
export function validateEmoteDeformBoneKeyframes(
  ...
  if (missingBones.length > 0) {
    issues.push({
      ...
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.emote_missing_keyframes',
      messageParams: {
        bones:
          missingBones.slice(0, 5).join(', ') + (missingBones.length > 5 ? ` (+${missingBones.length - 5} more)` : '')
      }
    })
  }
 ...
 */
export function validatePropTriangles(Three: ThreeModules, scene: Object3D): ValidationIssue[] {
  const issues: ValidationIssue[] = []
  let propTriangles = 0
  ...
  propArmature.traverse((node: Object3D) => {
  ...
  if (propTriangles > PROP_MAX_TRIANGLES) {
    issues.push({
      ...
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.prop_triangle_count',
 ...
 * Reports ERROR when exceeded.
 */
export function validatePropMaterials(Three: ThreeModules, scene: Object3D): ValidationIssue[] {
  const issues: ValidationIssue[] = []
  ...
  if (materials.size > PROP_MAX_MATERIALS) {
    issues.push({
      code: 'PROP_MATERIALS',
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.prop_materials',
 ...
 * Reports ERROR when exceeded.
 */
export function validatePropTextures(Three: ThreeModules, scene: Object3D): ValidationIssue[] {
  const issues: ValidationIssue[] = []
  ...
  if (textures.size > PROP_MAX_TEXTURES) {
    issues.push({
      code: 'PROP_TEXTURES',
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.prop_textures',
 ...
 * Reports ERROR when exceeded.
 */
export function validatePropArmatureBones(Three: ThreeModules, scene: Object3D): ValidationIssue[] {
  const issues: ValidationIssue[] = []
  let boneCount = 0
  ...
  if (boneCount > PROP_MAX_ARMATURE_BONES) {
    issues.push({
      ...
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.prop_armature_bones',
 ...
 */
export function validateArmatureNaming(scene: Object3D): ValidationIssue[] {
  const issues: ValidationIssue[] = []
  ...
  if (invalidNames.length > 0) {
    issues.push({
      ...
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.armature_naming',
...
export function validateAnimationNaming(animations: AnimationClip[], hasProps: boolean): ValidationIssue[] {
  ...
  if (invalidNames.length > 0) {
    issues.push({
      ...
      severity: ValidationSeverity.WARNING,
      messageKey: 'item_validation.animation_naming',
      messageParams: { names: invalidNames.join(', ') }
    })
  }
...
export function validateAudioFormat(contents: Record<string, Blob>): ValidationIssue[] {
  ...
  for (const key of Object.keys(contents)) {
    ...
    const isAudio =
      lowerKey.endsWith('.mp3') || lowerKey.endsWith('.ogg') || lowerKey.endsWith('.wav') || lowerKey.endsWith('.aac')
    ...
    if (isAudio) {
      ...
      if (!hasValidExt) {
        issues.push({
          code: 'AUDIO_FORMAT',
          severity: ValidationSeverity.WARNING,
          messageKey: 'item_validation.audio_format',
